...
namespace Baldini.Controls.Calendar
{
    ...
    public class CalendarRenderer
    {
        ...
        /// <summary>
        /// Possible corners to pass to theRoundRectangle method
        /// </summary>
        [Flags()]
        public enum Corners
        {
        ...
        /// <summary>
        /// Compares bothCalendarDayTop items by Date
        /// </summary>
        ...
        private string _dayHeaderFormatShort;
        private string _dayHeaderFormatExpanded;
        ...
        public CalendarRenderer(Calendar calendar)
        {
            ...
            _dayHeaderFormatShort = "d MMM";
            _dayHeaderFormatExpanded = "dddd d";
            _calendar = calendar;
            _allDayItemsPadding = 5;
            _itemsPadding = 5;
            _itemTextMargin = new Padding(1); //new Padding(3);
        ...
        /// <summary> Gets or Sets the DayHeaderFormat when Calendar.DaysMode = Expanded
        /// </summary>
        public string DayHeaderFormatExpanded
        {
            get
            {
                return _dayHeaderFormatExpanded;
            }
            set
            {
                _dayHeaderFormatExpanded = value;
                if (Calendar != null)
                    Calendar.Invalidate();
            }
        }

        /// <summary> Gets or Sets the DayHeaderFormat when Calendar.DaysMode = Short
        /// </summary>
        public string DayHeaderFormatShort
        {
            get
            {
                return _dayHeaderFormatShort;
            }
            set
            {
                _dayHeaderFormatShort = value;
                if (Calendar != null)
                    Calendar.Invalidate();
            }
        }
        ...
        /// when inCalendarDaysMode.Expanded
        /// </summary>
        ...
        /// <summary>
        /// Gets theCalendar this renderer belongs to
        /// </summary>
        ...
        /// <summary>
        /// Gets if theTimeScale is currently visible.
        /// </summary>
        ...
        /// <summary>
        /// Gets the width of the header of weeks, whenCalendar.DaysMode inCalendarDaysMode.Short
        /// </summary>
        ...
        /// This only works whenCalendar.DaysMode is in <c>Expanded</c> mode.
        /// </summary>
        ...
        public GraphicsPath ItemRectangle(CalendarRendererItemBoundsEventArgs evtData, Rectangle bounds)
        {
            ...
            //    //Point a = new Point(bounds.Left, bounds.Top);
        ...
        /// Optionally callsPerformItemsLayout to update bounds of items.
        /// </summary>
        /// <remarks>
        /// This method is called every time theCalendar control is resized.
        /// </remarks>
        public void PerformLayout(bool performItemsLayout)
        {
            ...
            if (Calendar.DaysMode == CalendarDaysMode.Expanded)
            {
                ...
                curLeft = !Calendar.HideTimeScale ? TimeScaleBounds.Right : 0;
                dayHeight = Calendar.ClientSize.Height - 1;
                dayWidth = !Calendar.HideTimeScale ? (Calendar.ClientSize.Width - TimeScaleBounds.Width - scrollBarWidth) / Calendar.Days.Length : (Calendar.ClientSize.Width - scrollBarWidth) / Calendar.Days.Length;
        ...
        public void PerformItemsLayout()
        {
            ...
            if (Calendar.DaysMode == CalendarDaysMode.Expanded)
            {
                ...
                foreach (CalendarItem item in Calendar.Items)
                {
                    ...
                    if (item.IsOnDayTop)
                    {
                        ...
                        {
                            //if (
                            //    (item.StartDate.Date >= dayStart.Date.Date && item.StartDate.Date <= dayEnd.Date.Date) ||
                            //    (item.EndDate.Date >= dayStart.Date.Date && item.EndDate.Date <= dayEnd.Date.Date))
                            //{
                    ...
                    else
                    {
                        ...
                        if (date1 == date2)
                            date2 = date1.AddMinutes(5);
                        if (unitDurationMinutes == 0)
                            unitDurationMinutes = Convert.ToDouble((int)CalendarTimeScale.ThirtyMinutes);
                ...
                if (tmatix.GetLength(1) > 0)
                {
                    ...
                    int itemsHeight = tmatix.GetLength(1) * StandardItemHeight;
                    int dayTopsHeight = itemsHeight >= DayTopMinHeight ? itemsHeight : DayTopMinHeight;

                    if (Calendar.DayTopHeightFixed)
                        dayTopsHeight = Calendar.MaximumDayTopHeight;

                    if (dayTopsHeight > Calendar.MaximumDayTopHeight && Calendar.MaximumDayTopHeight != 0)
                        dayTopsHeight = Calendar.MaximumDayTopHeight;
            ...
            else if (Calendar.DaysMode == CalendarDaysMode.Short)
            {
                ...
                foreach (CalendarItem item in Calendar.Items)
                {
                    if (item.DayStart == null || item.DayEnd == null) continue;
                ...
                for (int week = 0; week < Calendar.Weeks.Length; week++)
                {
                    ...
                    foreach (CalendarItem item in Calendar.Items)
                    {
                        ...
                        int rtop = rStart.Top + DayHeaderHeight + top * StandardItemHeight - dayStart.ItemIndex * StandardItemHeight ;
                        Rectangle r = Rectangle.FromLTRB(rStart.Left, rtop, rEnd.Right, rtop + StandardItemHeight);

                        if (r.Top <= sunday.Bounds.Top)
                            for (int i = dayStart.Index; i <= dayEnd.Index; i++)
                                Calendar.Days[i].SetOverflowStart(true);
                        else if (r.Bottom <= sunday.Bounds.Bottom)
                            item.AddBounds(r);
                        else
                            for (int i = dayStart.Index; i <= dayEnd.Index; i++)
        ...
        private static void PlaceInMatrix(ref int[,] m, int index, int startX, int endX)
        {
            int y = 0;
            bool yFound = false;
        ...
        /// <summary>
        /// Sets the value of theDayTopHeight property
        /// </summary>
        /// <param name="height">Height of allCalendarDayTop elements</param>
        ...
        /// <summary>
        /// Sets the value of theDayHeaderHeight property
        /// </summary>
        ...
        /// <summary>
        /// Sets the value of theDayNameHeadersHeight property
        /// </summary>
        ...
        /// <summary>
        /// Sets the value of theTimeScaleUnitHeight property
        /// </summary>
        ...
        /// <summary>
        /// Sets the value of theTimeScaleWidth property
        /// </summary>
        ...
        /// <summary>
        /// Draws text using the information of theCalendarRendererBoxEventArgs
        /// </summary>
        /// <param name="e"></param>
        protected virtual void DrawStandardText(CalendarRendererBoxEventArgs e)
        {
            TextRenderer.DrawText(e.Graphics, e.Text.Replace("&", "&&"), e.Font, e.Bounds, e.TextColor, e.Format);
        }
        ...
        private static void FindInMatrix(int[,] m, int number, out int left, out int top)
        {
        ...
        private static void GetGroupBoundUnits(List<CalendarItem> group, out int startIndex, out int endIndex)
        {
        ...
        private void CollectIntersectingGroup(CalendarItem calendarItem, List<CalendarItem> items, List<CalendarItem> grouped)
        {
            ...
            foreach (CalendarItem item in items)
            {
                if (!grouped.Contains(item) &&
                    calendarItem.IntersectsWith(item.StartDate.TimeOfDay, item.EndDate.TimeOfDay, calendarItem.Calendar.TimeScale))
                {
                    grouped.Add(item);
        ...
        {
            if (e.Calendar.DaysMode == CalendarDaysMode.Short || e.Calendar.HideTimeScale
            ...
            Font hourFont = new Font(e.Calendar.Font.FontFamily, e.Calendar.Font.Size * 1.5f);
            ...
            for (int i = 0; i < e.Calendar.Days[0].TimeUnits.Length; i++)
            {
                ...
                string hours = unit.Hours.ToString();//.ToString("00");
                string minutes = unit.Minutes == 0 ? "00" : string.Empty;

                if (!string.IsNullOrEmpty(minutes))
                {
                    //Show 12 for Midnight rather than 00
                    if (hours == "00" | hours == "0") hours = "12";
                    //Make sure the time is civilian time, not Military
                    if (int.Parse(hours) > 12) hours = (int.Parse(hours) - 12).ToString();//.PadLeft(2, '0');
                    CalendarRendererBoxEventArgs hevt = new CalendarRendererBoxEventArgs(e, new Rectangle(hourLeft + 3, unit.Bounds.Top, hourWidth, unit.Bounds.Height), hours, TextFormatFlags.Right);
                    ...
                    CalendarRendererBoxEventArgs mevt = new CalendarRendererBoxEventArgs(e, new Rectangle(minuteLeft - 5, unit.Bounds.Top + 3, minuteWidth, unit.Bounds.Height), minutes, TextFormatFlags.Left);//TextFormatFlags.Top | TextFormatFlags.Left);
        ...
        public virtual void OnDrawDay(CalendarRendererDayEventArgs e)
        {
            ...
            hevt.Font = new Font(Calendar.Font, FontStyle.Regular);
            ...
            if (e.Calendar.DaysMode == CalendarDaysMode.Resource)
                devt.Text = e.Day.Resource.ToString();

            OnDrawDayHeaderBackground(e);

            if (Calendar.DaysMode == CalendarDaysMode.Short && (day.Index == 0 || day.Date.Day == 1))
            {
                hevt.Text = day.Date.ToString(DayHeaderFormatShort);
            }
            if (Calendar.DaysMode == CalendarDaysMode.Expanded)
            {
                hevt.Text = day.Date.ToString(DayHeaderFormatExpanded);
            }

            if (Calendar.Holidays.ContainsKey(e.Day.Date))
            {
                hevt.Text += " - " + Calendar.Holidays.GetDescriptionForDay(e.Day.Date); //[e.Day.Date];
                hevt.Font = new Font(Calendar.Font, FontStyle.Bold);
                hevt.TextColor = Color.FromArgb(70, 91, 110);
                hevt.RemoveEnum(TextFormatFlags.WordBreak);
                hevt.AddEnum(TextFormatFlags.EndEllipsis);
            }

            OnDrawDayHeaderText(hevt);

            if (devt.TextSize.Width < day.HeaderBounds.Width - hevt.TextSize.Width * 2
                && (e.Calendar.DaysMode == CalendarDaysMode.Expanded && e.Calendar.DaysMode == CalendarDaysMode.Resource))
            {
        ...
        ///Calendar.DaysMode isCalendarDaysMode.Short
        /// </summary>
        /// <param name="e"></param>
        ...
        /// <summary>
        /// Paints the name of the day columns whenCalendar.DaysMode isCalendarDaysMode.Short
        /// </summary>
        /// <param name="e">Paint Info</param>
        ...
        /// <summary>
        /// Paints a name of the day column when Calendar.DaysMode is CalendarDaysMode.Short
        /// </summary>
        /// <param name="e">Paint info</param>
        ...
        public virtual void OnDrawItems(CalendarRendererEventArgs e)
        {
            ...
            #region Shadows
            var items = new List<CalendarItem>(e.Calendar.Items);
            items.RemoveAll((x) => x.StartDate >= e.Calendar.ViewEnd | x.EndDate <= e.Calendar.ViewStart);
            foreach (CalendarItem item in items)
            {
                clipped = false;
        ...
        /// <summary>
        /// Draws the pattern over the background of the specified item
        /// </summary>
        /// <param name="e">Event Info</param>
        ...
        public virtual void OnDrawItemContent(CalendarRendererItemBoundsEventArgs e)
        {
            ...
            for (int i = 0; i < rectangles.Count; i++)
            {
                ...
                //    rStartTime.Y = bounds.Top + ItemTextMargin.Top +(StandardItemHeight * i);
                ...
                if (e.Item.ShowStartTime || e.Item.ShowEndTime)
                    evt.Font = new Font(evt.Font, FontStyle.Regular); //FontStyle.Bold);

                if (e.Item.IsOnDayTop || Calendar.DaysMode == CalendarDaysMode.Short)
                    evt.Format = TextFormatFlags.SingleLine | TextFormatFlags.EndEllipsis;
                ...
                if (e.Item.Image != null)
                {
                    ...
                    // Limit Image size to a maximum of StandardItemHeight x StandardItemHeight
                    double h = e.Item.Image.Size.Height;
                    double w = e.Item.Image.Size.Width;
                    if ((h == w) && imageBounds.Height > StandardItemHeight - imgPadding)
                        imageBounds.Size = new Size(StandardItemHeight - (int)imgPadding, StandardItemHeight - (int)imgPadding);
                    else if ((h != w) && (imageBounds.Height > StandardItemHeight - imgPadding | imageBounds.Width > StandardItemHeight - imgPadding))
                    {
                        // We must compensate for the size differential when we shrink the image.
                        if (h > w)
                        { h = StandardItemHeight - imgPadding; w = (StandardItemHeight - imgPadding) * (w / h); }
                        else
                        { w = StandardItemHeight - imgPadding; h = (StandardItemHeight - imgPadding) * (h / w); }
                        imageBounds.Size = new Size((int)w, (int)h);
                    }

                    if ((e.Item.ImageAlign & CalendarItemImageAlign.North) == CalendarItemImageAlign.North && (e.Item.ImageAlign & CalendarItemImageAlign.East) != CalendarItemImageAlign.East)
                    { imageBounds.Y = tBounds.Y + ((int)imgPadding / 2); } //tBounds.Height -= imageBounds.Height;
                    if ((e.Item.ImageAlign & CalendarItemImageAlign.South) == CalendarItemImageAlign.South)
                    { imageBounds.Y = tBounds.Bottom - imageBounds.Height; } //tBounds.Height -= imageBounds.Height;
                    if ((e.Item.ImageAlign & CalendarItemImageAlign.East) == CalendarItemImageAlign.East)
                    { tBounds.Width -= imageBounds.Width; imageBounds.X = tBounds.Right - imageBounds.Width; }
                    if ((e.Item.ImageAlign & CalendarItemImageAlign.West) == CalendarItemImageAlign.West)
                    { tBounds.Width -= imageBounds.Width; imageBounds.X = tBounds.Left + ((int)imgPadding / 2); tBounds.X += imageBounds.Width; }

                    if (((e.Item.ImageAlign & CalendarItemImageAlign.East) != CalendarItemImageAlign.East &&
                        ((e.Item.ImageAlign & CalendarItemImageAlign.West) != CalendarItemImageAlign.West)))
                        imageBounds.X = e.Item.Bounds.X + ( ( e.Item.Bounds.Width - imageBounds.Width ) / 2);

                    if (((e.Item.ImageAlign & CalendarItemImageAlign.North) != CalendarItemImageAlign.North &&
                        ((e.Item.ImageAlign & CalendarItemImageAlign.South) != CalendarItemImageAlign.South)))
                        imageBounds.Y = e.Item.Bounds.Y + ((e.Item.Bounds.Height - imageBounds.Height) / 2);

                    e.Item.ImageBounds = imageBounds;
        ...
        /// <summary>
        /// Draws the Time Of Day of the current day
        /// </summary>
        /// <param name="e"></param>
        public virtual void OnDrawTimeOfDay(CalendarRendererEventArgs e)
        {

        }

        /// <summary>
        /// Paints the headers of the week rows whenCalendar.DaysMode isCalendarDaysMode.Short
        /// </summary>
        /// <param name="e"></param>
        ...
        /// <summary>
        /// Paints the header of a week row whenCalendar.DaysMode isCalendarDaysMode.Short
        /// </summary>
        /// <param name="e">Paint info</param>
    ...
    /// <summary>
    /// Contains information about aCalendarTimeScaleUnit that is about to be painted
    /// </summary>
    ...
    /// <summary>
    /// Contains basic information about a drawing event forCalendarRenderer
    /// </summary>
        ...
        : EventArgs
    {
        ...
        /// <summary>
        /// Creates a newCalendarRendererEventArgs
        /// </summary>
        ...
        /// <summary>
        /// Copies the parameters from the specifiedCalendarRendererEventArgs
        /// </summary>
        /// <param name="original"></param>
        ...
        : CalendarRendererEventArgs
    {
        ...
        /// <summary>
        /// Creates a newCalendarRendererDayEventArgs object
        /// </summary>
        ...
        : CalendarRendererEventArgs
    {
        ...
        #region Methods
        internal void RemoveEnum(TextFormatFlags flag)
        {
            this._format = this._format ^ flag;
        }

        internal void AddEnum(TextFormatFlags flag)
        {
            if ((this._format & flag) != flag)
                this._format = this.Format | flag;
        }
        #endregion
    }
}
